...
| Agency-account selector / typeahead for impersonation start | locked-by-ADR | Backend `GET /admin/agencies?q=...` gated by `IMPERSONATE_AGENCY_ACCOUNT`, returning tenant-scoped AGENCY accounts; UI replaces raw ULID input with combobox. Replaces the V1 raw-ULID workflow. | (planned: backend slice + UI slice) | ADR-027 |
