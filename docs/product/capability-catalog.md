...
| Operator impersonation UI (banner + start/stop/active page) | locked-by-ADR | ADR-027 D10/D11 surface in `apps/admin`. Persistent `<Banner variant="danger">` mounts in the `<SystemBanner />` slot that ADR-029 step 6 put in place. ADR-029 foundation is complete; this is the immediate next frontend slice. | (planned: `apps/admin/app/impersonation/`) | ADR-026, ADR-027, ADR-029 |
