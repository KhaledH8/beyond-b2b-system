...
| DB roles (`bb_app`, `bb_audit_retention`, `bb_admin`) | implemented | Idempotent `DO $$ … $$` migration creates three roles. `bb_app` INSERT/SELECT on `audit_event`; `bb_audit_retention` INSERT/SELECT on pruning log; `bb_admin` DDL only. Triggers are the sole append-only enforcer in local dev/CI (where the app connects as owner). | `infra/migrations/audit/20260509000001_audit_roles.ts` | ADR-028 |
| Canonical `audit_event` substrate | implemented | DB-role-enforced append-only; composite-partitioned by category × month; five categories; append-only triggers on parent; indexes on actor, target, request, kind; current+next month leaf partitions created at migration time. | `infra/migrations/audit/20260509000002_audit_event.ts` | ADR-028 |
| `audit_pruning_log` | implemented | Standalone never-pruned table recording every partition drop; `bb_audit_retention` INSERT-only. | `infra/migrations/audit/20260509000003_audit_pruning_log.ts` | ADR-028 D2.d |
| `AuditService` with category-aware emission | implemented | Compile-time-enforced split via `AuditEventInputBackground = Extract<..., {category: 'APP'\|'SECURITY'}>`. Runtime guard throws for AUTH/IMPERSONATION/SENSITIVE_ACCESS if type cast bypasses TS. `emit()` swallows errors; `emitInTransaction()` propagates (transaction rolls back). Reads `RequestAuditContext` from AsyncLocalStorage. | `apps/api/src/audit/audit.service.ts`, `apps/api/src/audit/audit.module.ts` | ADR-028 D7 |
| `RequestIdMiddleware` — request-id propagation | implemented | Mints ULID per request (or validates existing `X-Request-Id` via Crockford base32 pattern). Sets `X-Request-Id` on response. Runs `requestContextStore.run()` so all downstream code shares the `RequestAuditContext` via AsyncLocalStorage. Extracts client IP (X-Forwarded-For first) and user-agent. | `apps/api/src/audit/request-id.middleware.ts` | ADR-028 D6 |
...
| Booking confirmation issue (`POST /internal/documents/booking-confirmation`) | implemented | Slice 1: structured-JSON `BB_BOOKING_CONFIRMATION` for a CONFIRMED booking; content from immutable booking-time snapshots only; content-addressed JSON blob in object storage; monotonic-per-tenant number (`BB-CONF-YYYY-NNNNN`, not gapless); immutable ISSUED row; durable `BOOKING_DOCUMENT_CREATED` audit; idempotent on `(bookingId, type)`. Documents-owned endpoint (ADR-011: `booking` does not import `documents`). No PDF, voucher, invoice, email, reseller branding, or async worker yet. | `apps/api/src/documents/` | ADR-016, ADR-011, ADR-028 |
| `doc_number_sequence` / `doc_booking_document` tables | implemented | Slice 1: per-`(tenant, type, scope_key)` monotonic counter + one immutable issued document per `(booking_id, type)`. `doc_legal_entity` / `doc_delivery_attempt` / tax-invoice tables deliberately not yet created. | `infra/migrations/documents/` | ADR-016 |
...
| Audit read API LIST (`GET /admin/audit/events`) | implemented | JWT + `RolesGuard` + `AUDIT_READ` (NOT `InternalAuthGuard`). Tenant-scoped via `AuthContext.tenantId`. Filters: category / kind / actorUserId / target / requestId / impersonationGrantId / occurredFrom / occurredTo. Cursor pagination on `(occurred_at, id)`. Default limit 50, max 200. `SENSITIVE_ACCESS` category requires `AUDIT_READ_SENSITIVE` (held only by `platform_admin`); base callers see those rows silently filtered out. Emits `SECURITY.AUDIT_QUERY_EXECUTED` on every successful call (background, best-effort). | `apps/api/src/admin-audit/` | ADR-028 D9, ADR-026 D6 (amended 2026-05-13) |
| Audit read API DETAIL (`GET /admin/audit/events/:id`) | locked-by-ADR | Single-event fetch; emits `AUDIT_QUERY_EXECUTED_SENSITIVE` when the fetched event is in `SENSITIVE_ACCESS` (or a future PII-flagged kind). | (planned: ADR-028 step 7b) | ADR-028 D9 |
| Audit CLI (`bb-audit query`) | locked-by-ADR | Same query surface as the LIST API; emits `AUDIT_QUERY_EXECUTED` via `AuditService`. | (planned: ADR-028 step 8) | ADR-028 D9 |
| Audit retention cron | locked-by-ADR | Per-category monthly leaf-partition drop; writes `audit_pruning_log` before each `DROP TABLE`; emits `SECURITY.AUDIT_PARTITION_DROPPED`. | (planned: ADR-028 step 9) | ADR-028 D8 |
| `SENSITIVE_ACCESS` audit emitters (V1.1) | locked-by-ADR | Annotates impersonation sessions when a flagged surface is read; writes to the `SENSITIVE_ACCESS` partition tree. | (planned: ADR-028 step 11) | ADR-027 D9 Layer 3, ADR-028 D4 |
| Admin app foundation (auth/session/API client/layout/design system v0) | implemented | Auth0 Universal Login via `@auth0/nextjs-auth0` v4; single `lib/session.ts` with `requireOperatorSession()`; single server-side `lib/api-client.ts` (no-store, bearer auto-attached, typed error classes, no retry); operator-only layout gate (AGENCY users 403); Tailwind CSS v4 + 5-component v0 (`Button`, `Input`, `Textarea`, `Card`, `Banner`); `AdminShell` + `Header` + `Sidebar` + `SystemBanner` layout; single-tenant per deployment via `BB_TENANT_ID`; 135 admin tests; no `offline_access`; no dev-token bypass. All 7 steps shipped 2026-05-10. | `apps/admin` | ADR-026, ADR-027, ADR-029 |
| Operator impersonation UI v1 (banner + start/stop/active page) | implemented | ADR-027 D10/D11 surface in `apps/admin`. Persistent `<Banner variant="danger">` mounts in the `<SystemBanner />` slot (account name + ID + ticketRef + expiry + READ_ONLY warning + Stop). `/impersonation` page renders an active card or a three-field start form (ULID + ticketRef + reasonText). Server actions own the start/stop calls. Typed `lib/impersonation-client.ts` wraps `apiFetch`. **V1 limitation: raw 26-char ULID input — no agency selector / typeahead yet.** Backend still validates AGENCY + same-tenant. | `apps/admin/app/(protected)/impersonation/`, `apps/admin/components/Impersonation*.tsx`, `apps/admin/lib/impersonation-client.ts` | ADR-026, ADR-027, ADR-029 |
| Agency-account selector for impersonation start (V1.1) | implemented | `GET /admin/agencies?q=...&limit=…` gated by `IMPERSONATE_AGENCY_ACCOUNT`. JWT + `RolesGuard`, NOT `InternalAuthGuard`. Tenant scope from `AuthContext.tenantId`. Hard WHERE: `account_type = 'AGENCY'` + `status = 'ACTIVE'`. Search by name (ILIKE substring) or ID (ILIKE prefix). `limit` clamped 1–50, default 20. Admin UI replaces V1 raw-ULID-only input with a server-rendered list + Search button; manual ULID fallback retained. | `apps/api/src/admin-agencies/`, `apps/admin/components/ImpersonationStartForm.tsx`, `apps/admin/lib/impersonation-client.ts` (`listAgencies`) | ADR-027 |
